...
const assignorService = {
  getAll: async (page: number = 1, limit: number = 100): Promise<Assignor[]> => {
    const response = await apiService.get<PaginatedResponse<Assignor>>(`/integrations/assignor?page=${page}&limit=${limit}`);
